...
import { clearLastLine } from './utils.ts';

export async function write(end: 'front' | 'back', options: Configuration, username: string) {
  ...
  total =
    Object.keys(await getFiles(`${base}assets/out/${options.platform}-template/${options.script}${end === 'front' ? '/' + options.style : ''}.json`)).length -
    1 +
    4;
  ...
  if (options.platform !== 'oak') {
    ...
    const hpDir =
      options.platform === 'next' ? path.resolve('pages/index' + extension) : options.platform === 'react' ? path.resolve('src/App' + extension) : '';
