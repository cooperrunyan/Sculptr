...
function writeTs(overwrite: boolean, strict: boolean): void {
  ...
  if (!exists) fs.copyFileSync(path.resolve(`${root}/../assets/files/tsconfig${strict ? '-file' : '-loose'}.json`), './tsconfig.json');
  else {
    if (!overwrite) {
      const tsc = JSON.parse(fs.readFileSync(path.resolve('./tsconfig.json'), 'utf-8'));
      const newTsc = JSON.parse(fs.readFileSync(path.resolve(`${root}/../assets/files/tsconfig${strict ? '-file' : '-loose'}.json`), 'utf-8'));

      tsc.compilerOptions = { ...tsc.compilerOptions, ...newTsc.compilerOptions };

      fs.writeFileSync('./tsconfig.json', JSON.stringify(tsc, null, 2));
    } else fs.writeFileSync('./tsconfig.json', fs.readFileSync(path.resolve(`${root}/../assets/files/tsconfig${strict ? '-file' : '-loose'}.json`), 'utf8'));
  }
...
function writeSass() {
  copyFolderRecursiveSync(path.resolve(`${root}/../../../assets/files/sass`), path.resolve('sass'));
}

function writeScss() {
  copyFolderRecursiveSync(path.resolve(`${root}/../../../assets/files/scss`), path.resolve('scss'));
}
